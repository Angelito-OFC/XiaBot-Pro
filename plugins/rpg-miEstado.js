...
let sections = Object.keys(emoji, significado).map((v, index) => ({ title: `🤔 𝘏𝘰𝘭𝘢 𝘩𝘶𝘮𝘢𝘯𝘰 𝘤𝘰𝘮𝘰 𝘦𝘴𝘵𝘢𝘴 𝘦𝘭 𝘥𝘪𝘢 𝘥𝘦 𝘩𝘰𝘺?`,
...
if (command == 'miestado') { 
if (!text) return conn.sendMessage(m.chat, {text: `❖ 𝘏𝘰𝘭𝘢 𝘩𝘶𝘮𝘢𝘯𝘰 𝘤𝘰𝘮𝘰 𝘦𝘴𝘵𝘢𝘴 𝘦𝘭 𝘥𝘪𝘢 𝘥𝘦 𝘩𝘰𝘺? 🤔 ${user.registered === true ? user.name : name}\n\n*╭⸺ ⊹ ⸺  ⊹ ⸺ ⊹ ⸺ ⊹ ⸺ ⊹ 》*\n👋 *SELECCIONE SU ESTADO ACTUAL POR FAVOR*\n*❖ SU ESTADO ACTUAL:* ${typeof user.miestado === 'string' ? user.miestado : 'Estado no asignado\n❖ Ejemplo /miestado 😃'}\n*╰⸺ ⊹ ⸺  ⊹ ⸺ ⊹ ⸺ ⊹ ⸺ ⊹ 》*`}, {quoted: fkontak})
miEstado = text.trim()
